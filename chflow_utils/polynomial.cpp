...
Polynomial::Polynomial(int order) : c(order + 1) {}

Polynomial::Polynomial(const Vector &coeffs) : c(coeffs) {}

Polynomial::Polynomial(const Polynomial &p) : c(p.c) {}
...
// Polynomial &Polynomial::operator=(const Polynomial &p) { c = p.c; }

Polynomial &Polynomial::operator=(const Polynomial &p) {
  if (this == &p)
    return *this; // Self-assignment check
                  //
  cout << "Assigning Polynomial, size = " << p.c << endl;

  c = p.c; // Assuming `c` is a std::vector or other safe-to-copy type

  return *this;
}
int Polynomial::order() const { return c.length() == 0 ? 0 : c.length() - 1; }

int Polynomial::length() const { return c.length(); }
...
//  p(x) = a0 + a1 x + a2 x^2
//  q(x) = b0 + b1 x + b2 x^2
// pq(x) = a0*b0 + (a0 b1 + a1 b0) x + (a0 b2 + a1 b1 + a2 b0) x^2 + (a1 b2 +
// a2 b1 + a2 b0) x^3 + a2 b2 x^4
...
Real L2innerproduct(const Polynomial &p, const Polynomial &q) {
  ...
  return 0.5 *
         (integral_pq(1.0) - integral_pq(-1.0)); // HARDCODED for domain [-1,1]
}

Real L2Norm2(const Polynomial &p) { return L2innerproduct(p, p); }
...
Real L2Dist(const Polynomial &p, const Polynomial &q) {
  return sqrt(L2Dist2(p, q));
}
